...
export class EdgeConfig {
  constructor(edge: Edge, source?: EdgeConfig) {
    ...
    if (
      ...
      Object.keys(this.factories).length == 0
    ) {
   ...
   * @param nature the given Nature.
   */
  public getComponentsImplementingNature(
    natureId: string
  ...
  public hasStorage(): boolean {
    if (
      this.getComponentIdsImplementingNature(
  ...
  public hasProducer(): boolean {
    ...
    if (
      this.getComponentsImplementingNature(
    ...
    for (let component of this.getComponentsImplementingNature(
      'io.openems.edge.meter.api.SymmetricMeter'
    )) {
    ...
    let allFactories = [
      {
        ...
        factories: Object.values(this.factories).filter((factory) =>
          factory.id.startsWith('Simulator.')
        ),
      },
      {
      ...
      {
        category: {
          title: 'Speichersystem-Steuerung',
      ...
      },
      {
        ...
        factories: [this.getFactoriesByNature('io.openems.edge.evcs.api.Evcs')],
      },
      {
        category: {
          title: 'E-Auto-Ladestation-Steuerung',
          icon: 'options-outline',
        },
        factories: [this.getFactoriesByIds(['Controller.Evcs'])],
      },
      {
    ...
    allFactories.forEach((item) => {
      ...
      if (factories.length > 0) {
        ...
        result.push({
          ...
          factories: factories.sort((a, b) => a.id.localeCompare(b.id)),
        });
      }
    });
    return result;
